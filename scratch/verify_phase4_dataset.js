...
function runHeadlessRuntimeVerification() {
  ...
  console.log("=== HEADLESS RUNTIME VERIFICATION COMPLETE ===");
  console.log(`Initial Route ID: ${initialRouteId}`);
  console.log(`Final Route ID: ${state.navigation.activeRoute.id}`);
  console.log(`Route Changes Count: ${routeChanges}`);
  console.log(`Planner Calls Count: ${episodeObj.route_events.length}`);
  console.log(`Ship Heading Before Avoidance: 330°`);
  console.log(`Peak Heading Change: ${Math.abs(ship.heading - 330).toFixed(1)}°`);
  console.log(`Minimum Iceberg Clearance: ${episodeObj.metrics.minIcebergClearance} SU`);
  console.log(`Collision Result: ${ship.lastCollisionEvent.collisionDetected}`);
  console.log(`Route Flapping Score: ${episodeObj.metrics.routeFlappingScore}`);
  console.log(`Data Quality Valid: ${episodeObj.data_quality.valid}`);
  console.log(`Export JSON Path: ${exportPath}`);
}

runHeadlessRuntimeVerification();
